...
export default function AdminProducts() {
  ...
  const handleImageSelect = (e) => {
    ...
    setCropQueue(files); // crop one at a time; upload happens as each crop is applied
  };

  const handleProductCropApply = async (blob) => {
    const current = cropQueue[0];
    const replaceIndex = current && current.__replaceIndex;
  ...
  const handleProductCropSkip = () => setCropQueue((q) => q.slice(1));
    ...
    <div>
      {cropQueue.length > 0 && (
        <ImageCropModal
          ...
          aspect={1}
          outputWidth={900}
          onCancel={handleProductCropSkip}
                ...
                {form.images.map((url, idx) => (
                  <div key={url} className="relative">
                    ...
                    <button
                      type="button"
                      title="Reposition"
                      className="absolute -bottom-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-slate-700 text-[10px] text-white"
                      onClick={async () => {
                        const res = await fetch(url);
                        const blob = await res.blob();
                        setCropQueue([Object.assign(blob, { __replaceIndex: idx })]);
                      }}
                    >
                      ✎
                    </button>
