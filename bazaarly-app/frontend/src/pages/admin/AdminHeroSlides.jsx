...
// Wide banner ratio — close enough to both the desktop and mobile hero box
// that "cover" fit no longer needs to guess where to crop; the admin already
// framed the shot themselves.
const HERO_CROP_ASPECT = 2.4;
...
export default function AdminHeroSlides() {
  ...
  const [cropFile, setCropFile] = useState(null); // file picked, waiting to be positioned
  ...
  const startEdit = (s) => {
    ...
    setForm({
      ...
      title: s.title || '',
      subtitle: s.subtitle || '',
      specsText: (Array.isArray(s.specs) ? s.specs : []).join(', '),
      ctaText: s.ctaText || '',
      ctaLink: s.ctaLink || '',
  ...
  const handleImageSelect = (e) => {
    ...
    if (!file) return;
    setCropFile(file); // opens the crop modal; upload happens after the crop is applied
  };
